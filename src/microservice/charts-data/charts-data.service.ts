...
import { ChartPoint, Record, SessionData } from '@common/types'
...
export class ChartsDataService {
  ...
  create(points: ChartPoint[], origLength: number, workoutId: number) {
  ...
  async findChartsDataById(id: number | string) {
    return this.chartPointRepository.findByWorkoutId(id)
  }

  makeChartsData(
    records: Record[],
    session: SessionData,
  ): { chartPoints: ChartPoint[]; mutatedSession: SessionData } {
    try {
      const points: ChartPoint[] = []
      const mutatedSession: SessionData = { ...session }
